...
- **公開エッジ(ブラウザ SPA・将来の 3rd party / webhook)= REST + OpenAPI。** spec-first(`api/openapi.yaml`)で oapi-codegen が Go サーバ interface を、openapi-typescript が TS クライアント(`packages/api-client`)を生成。フロントに Connect は出さない。
